...
export class ContentService {
  constructor(
    private readonly generateUseCase: GenerateContentUseCase,
    private readonly selectUseCase: SelectContentUseCase,
    @Inject('ContentRepository') private readonly repo: IContentRepository,
  ) {}

  async generate(prompt: string, type: 'POST' | 'STORY'): Promise<Content> {
    return this.generateUseCase.execute({ prompt, type });
  }

  async select(contentId: string, selected: 'A' | 'B'): Promise<void> {
    await this.selectUseCase.execute({ contentId, selected });
  }
